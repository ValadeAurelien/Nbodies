                               ...
                               args_t args, int seed)
{
  ...
  switch (args.speed_type)
  {
    ...
    case WRAP_X :
      bodies_1[j*X+i].vx = 0;
      bodies_1[j*X+i].vy = args.speed_norm*(- bodies_1[j*X+i].z/args.max_Z + args.max_Z/2);
      bodies_1[j*X+i].vz = args.speed_norm*(bodies_1[j*X+i].y/args.max_Y - args.max_Y/2);
      break;
    case WRAP_Z :
      bodies_1[j*X+i].vz = 0;
      bodies_1[j*X+i].vy = args.speed_norm*(- bodies_1[j*X+i].x/args.max_X + args.max_X/2);
      bodies_1[j*X+i].vx = args.speed_norm*( bodies_1[j*X+i].y/args.max_Y - args.max_Y/2);
      break;
    default :
      break;
  }
